...
@implementation IdentOptionsXMLRequest
...
- (IdentOptionsXMLRequest *)initWithURL:(NSString *)urlString delegate:(NSObject *)theDelegate
{
    self = [super init];
    ...
    if ( self ) {
        ...
        downloadStatus = IdentOptionsXMLRequestStatusNotStarted;
...
- (void)sendRequest {
    DBLog(@"IdentOptionsXMLRequest: sendRequest");
...
- (void)sendRequestWithDelay {
    DBLog(@"IdentOptionsXMLRequest sendRequestWithDelay");
...
- (void)sendRequestAndParseResponse {
    ...
    downloadStatus = IdentOptionsXMLRequestStatusInProgress;
    DBLog(@"IdentOptionsXMLRequest: making request with URL: %@", requestURLString);
    ...
    DBLog(@"IdentOptionsXMLRequest: parse call finished");
...
{
    DBLog(@"IdentOptionsXMLRequest: parsingDidTimeout");
    [[NetworkStatusHandler sharedInstance] networkTimeoutExceeded];
}

- (void)parserDidStartDocument:(NSXMLParser *)parser
{
    DBLog(@"IdentOptionsXMLRequest: parserDidStartDocument");
    downloadStatus = IdentOptionsXMLRequestStatusComplete;
    [[NetworkStatusHandler sharedInstance] removeRequester:self];
}

- (void)parser:(NSXMLParser *)parser parseErrorOccurred:(NSError *)parseError
{
    DBLog(@"IdentOptionsXMLRequest parseErrorOccurred: XMLParser failed! Error - %@ %@",
    ...
    {
        DBLog(@"IdentOptionsXMLRequest parseErrorOccurred: network problem?");
        downloadStatus = IdentOptionsXMLRequestStatusFailed;
        [parser abortParsing];
        [parser setDelegate:nil];
        
        if ([[NetworkStatusHandler sharedInstance] serverIsReachable])
        {
            DBLog(@"IdentOptionsXMLRequest parseErrorOccurred: retrying");
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict {
    ...
    if ( [elementName isEqualToString:@"response"]) {
        ...
        else
        {
            ...
            DBLog(@"IdentOptionsXMLRequest: parsing restarted apparently... NOT GOOD!");
        }
        return;
    }
    else if ( [elementName isEqualToString:@"identOption"] )
    {
        if (!identOptions)
        {
            identOptions = [[NSMutableArray alloc] init];
            [response setObject:identOptions forKey:@"identOption"];
        }
		if (!currentIdentOption)
		{
            currentIdentOption = [[NSMutableDictionary alloc] initWithCapacity:6];
        }
    }
}
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName {
    ...
    else if ( [elementName isEqualToString:@"identOption"])
    {
        ...
        if(currentIdentOption != nil)
        {
            [identOptions addObject:currentIdentOption];
            [currentIdentOption release];
            currentIdentOption = nil;
        }
    }
	else
	{
        if(currentStringValue != nil)
        {
            ...
            else
            {
                ...
                [currentIdentOption setObject:currentStringValue forKey:elementName];
            }
        }
    }
...
- (void)responseComplete {
    ...
    DBLog(@"IdentOptionsXMLRequest: Request status:%d", (int)requestSucceeded);
...
{
    if(network && (downloadStatus == IdentOptionsXMLRequestStatusFailed))
    {
        // Attempt restart
        [self sendRequest];
    }
