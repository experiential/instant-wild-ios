...
@implementation ImageListXMLRequest
...
- (ImageListXMLRequest *)initWithURL:(NSString *)urlString delegate:(NSObject *)theDelegate
{
    self = [super init];
...
- (void)sendRequest {
    DBLog(@"ImageListXMLRequest: sendRequest");
...
- (void)sendRequestWithDelay {
    DBLog(@"ImageListXMLRequest sendRequestWithDelay");
...
- (void)sendRequestAndParseResponse {
    ...
    DBLog(@"ImageListXMLRequest: making request with URL: %@", requestURLString);
    ...
    DBLog(@"ImageListXMLRequest: parse call finished");
...
{
    DBLog(@"ImageListXMLRequest: parsingDidTimeout");
    [[NetworkStatusHandler sharedInstance] networkTimeoutExceeded];
}

- (void)parserDidStartDocument:(NSXMLParser *)parser
{
    DBLog(@"ImageListXMLRequest: parserDidStartDocument");
...
{
    DBLog(@"ImageListXMLRequest parseErrorOccurred: XMLParser failed! Error - %@ %@",
    ...
    {
        DBLog(@"ImageListXMLRequest parseErrorOccurred: network problem?");
        ...
        {
            DBLog(@"ImageListXMLRequest parseErrorOccurred: retrying");
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict {
    ...
    if ( [elementName isEqualToString:@"response"])
    {
        ...
        else
        {
            ...
            DBLog(@"ImageListXMLRequest: parsing restarted apparently... NOT GOOD!");
        }
        return;
    }
    else if ( [elementName isEqualToString:@"image"] )
    {
        if (!images)
        {
            ...
            [response setObject:images forKey:@"image"];
        }
		if (!currentImage)
		{
	...
	else
	{
        if(currentStringValue != nil)
        {
            ...
            else
            {
                ...
                [currentImage setObject:currentStringValue forKey:elementName];
            }
        }
    }
...
- (void)responseComplete {
    ...
    DBLog(@"ImageListXMLRequest: Request status:%d", (int)requestSucceeded);
