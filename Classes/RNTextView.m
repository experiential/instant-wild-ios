...
- (void)finishInit
{
    //CGContextConcatCTM(UIGraphicsGetCurrentContext(), CGAffineTransformMakeScale(1.0, -1.0));
    //DBLog(@"RNTextView: completed finishInit");
}

- (void)dealloc
{
	[attributedString_ release], attributedString_ = nil;
	if (framesetter_ != NULL)
	{
		CFRelease(framesetter_), framesetter_ = NULL;
	}
	[paths_ release], paths_ = nil;
	
	if (subpaths != NULL)
	{
		CFRelease(subpaths), subpaths = NULL;
	}
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextConcatCTM(UIGraphicsGetCurrentContext(), flip);
    //CGContextConcatCTM(UIGraphicsGetCurrentContext(), CGAffineTransformMakeScale(1.0, -1.0));
	...
	{
		CGPathRef pathRef = (CGPathRef)path;
		CFArrayRef subpaths = [self copyRectangularPathsForPath:pathRef height:10.0];	// FIXME: Need algorithm w/o height. Probably means combining this routine with drawFromIndex:
...
-(void)setSubpaths:(CFArrayRef)subpathsArgument
{
    subpaths = subpathsArgument;
}
...
- (CTFramesetterRef)framesetter
{
    if (framesetter_ == NULL)
	{
        framesetter_ = CTFramesetterCreateWithAttributedString((CFAttributedStringRef)[self attributedString]);
    }
    return framesetter_;
}
